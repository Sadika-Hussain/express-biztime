...
// Returns details of a single company including its invoices (if any)
router.get("/:code", async function (req, res, next) {
    try {
        const code = req.params.code;

        // Use LEFT JOIN here to include companies without invoices in the result
        const results = await db.query(
            `SELECT 
                c.code, 
                c.name, 
                ...
                i.add_date AS invoice_add_date, 
        ...
        const { code: companyCode, name, description } = results.rows[0];
